...
$dobText = date('Y-m-d', strtotime($obj['dobText']));
$sql = "SELECT userid FROM member WHERE mobileno = '".$mobileno."' ";
//query to fetch
$userid = mysqli_fetch_array(mysqli_query($con,$sql));


$Sql_Query1="SELECT deviceeventid, MIN(time) AS 'FROM', MAX(time) AS 'TO' FROM devicefetchlogs WHERE date = '".$dobText."' AND detail1 = '".$userid['userid']."' ";
