   ...
   public function index(Request $request)
    ...
    {   
            ...
            if ($request->isMethod('post')){
              $request->validate([
                'Terms' => 'required|unique:terms,terms',
                'Minutes' => 'max:255',
            ],
            ['Terms.unique' => 'Term is already exists' ]);	
    ...
    if($usr){
      return redirect()->back()->withErrors('Terms Already exists');
         ...
         return redirect('terms')->with('message','Term is succesfully added');
        }
      ...
      $method = $request->method();
       $term=Terms::findOrFail($id);
            ...
            $term->terms=$request->Terms;
              ...
              $term->save();
         return redirect('terms')->with('message','Term is succesfully Edited');
        }
