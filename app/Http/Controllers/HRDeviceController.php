...
class HRDeviceController extends Controller
{
    public function adddevice(Request $request){
    	...
    	if($request->ismethod('post')){
    		...
    		$request->validate([
    			...
    			'devicename' => 'required|max:255|unique:hr_deviceinfo,devicename',
    		...
    		$ifexist = HRDeviceInfo::where('ipaddress', $ipaddress)->first();
            ...
            try {
        		...
        		return redirect()->route('hr_viewdevice');
    ...
    public function updatedevice($id, Request $request){
    	...
    	$device = HRDeviceInfo::findOrfail($id);
    	...
    	if($request->isMethod('post')){
    		...
    		$request->validate([
    			...
    			'deviceip_4' => 'required|digits_between:1,3|numeric',
    			'device_port' => 'required|digits_between:1,4|numeric',
    			'devicename' => ['required','max:255', Rule::unique(' hr_deviceinfo')->ignore($id, 'deviceinfoid')],
    			'username' => 'required|max:255',
    			'password' => 'required|max:255',
    		...
    		$ifexist = HRDeviceInfo::where('ipaddress', $ipaddress)->where('deviceinfoid', '!=', $id)->first();
