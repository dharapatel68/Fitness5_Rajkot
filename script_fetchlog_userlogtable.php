...
$deviceeventlastid = "SELECT * FROM hr_deviceevent where eventid='101' ORDER BY deviceeventid DESC LIMIT 1";
...
$devicefetchlogs = "SELECT * FROM hr_devicefetchlogs ORDER BY seqno DESC LIMIT 1";
...
if ($fno != $dresult['seqno']) {
  for($i=$fno;$i<=$dresult['seqno'];$i++)
    ...
    $fsino = implode(',', $sino);
    $sql1 = "SELECT * FROM hr_deviceevent where seqno IN (".$fsino.") AND eventid='101'";
	...
	while($row=mysqli_fetch_assoc($result1)){
     ...
     foreach ($dd as $ddd) {
      ...
      $date = date('Y-m-d',strtotime($ddd['date']));
     	...
     	$query1 = "INSERT INTO hr_devicefetchlogs (`rollovercount`,`seqno`,`date`,`time`,`eventid`,`detail1`,`detail2`,`detail3`,`detail4`,`detail5`) VALUES ('".$ddd['rollovercount']."','". $ddd['seqno']."','".$date."','".$ddd['time']."','".$ddd['eventid']."','".$ddd['detail1']."','".$ddd['detail2']."','".$ddd['detail3']."','".$ddd['detail4']."','".$ddd['detail5']."')";
