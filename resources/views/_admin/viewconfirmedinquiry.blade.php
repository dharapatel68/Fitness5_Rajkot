...
<!--   <form class="form-inline" method="post" action="{{ url('inquiry') }}">
  ...
  <div class="form-group">
    ...
    <div class="input-group date" style="max-width:180px" id="startdate">
      <input type="date" onkeypress="return false" class="form-control" name="inquirydate" placeholder="Inquiry Date"/></div>
    ...
    <div class="input-group date" style="max-width:180px" id="enddate">
      <input type="date" onkeypress="return false" class="form-control" name="followupdate" placeholder="To Date" />
...
<div class="table-wrapper">
  <div class="table-title">
    ...
    <div class="box-header">
      <a href="{{ url('inquiry') }}" class="btn add-new bg-navy"><i class="fa fa-eye"></i>View Inquiry</a>
                ...
                <tbody>@if($inqs)
                  @foreach($inqs as $inq)
                    ...
                    <tr>
                        ...
                        <td> {{ \Carbon\Carbon::parse($inq->updated_at)->format('d-m-Y')}}</td>
                        <td>{{$inq->firstname}} </td>
                        <td> {{$inq->lastname }}</td>
                        <td> {{ $inq->gender }}</td>                     
                        <td> {{ $inq->email }}</td>
                        ...
                        <td>
                            ...
                            <a href="{{ url('convertmember/'.$inq->inquiriesid) }}" class="check" title="convert into Member"><i class="fa fa-user"></i></a>
                              @php $code=str_random(6); @endphp
                              <a href="{{ url('sendmemberform/'.$inq->mobileno.'/'.$code) }}" class="check" title="Send Member Form"><i class="fa fa-times"></i></a>
