...
<div class="content-wrapper">
   ...
   <section class="content">
      ...
      <form role="form" action="{{ url('remainingpaymentstore') }}" name="frmMr" method="POST" id="package_form">
         {{ csrf_field() }}
         <input type="hidden" name="transactionId" value="{{ $transaction->transactionid }}">
         <input type="hidden" name="RootSchemeId" value="{{ $rootscheme->rootschemeid }}">
         <input type="hidden" name="SchemeID" value="{{ $scheme->schemeid }}">
         <input type="hidden" name="payment_id" value="{{ $payment_id }}">
         <input type="hidden" name="remainingamount_transaction" value="{{ $remainingamount_transaction }}">
         <input type="hidden" name="oldpaymentTransactionId" value="{{ $paymentTransactionId }}">
         ...
         <div class="box box-primary" id="secondstep" >
            <div class="box-header with-border">
               <h3 class="box-title">Remaining Payment Transcation</h3>
            </div>
            <!-- /.box-header -->
               ...
               <div class="col-md-8">
                 <table class="table">
                   <tr>
                     <th>Member Name</th>
                     <td>:</td>
                     <td>{{ !empty($member->firstname) ? ucfirst($member->firstname) : '' }} {{ !empty($member->lastname) ? ucfirst($member->lastname) : ''  }}</td>
                   </tr>
                    <tr>
                    ...
                    <tr>
                     <th>Scheme Name</th>
                     <td>:</td>
                     <td>{{ !empty($scheme_name) ? ucfirst($scheme_name) : '' }}</td>
                   </tr>
                   {{--  <tr>
                     <th>Base Amount</th>
                     <td>:</td>
                   ...
                   <tr>
                     ...
                     <td>:</td>
                     <td><b><span style="color: red;">{{ $transaction->transactionremainingamount }}</span></b></td>
                   </tr>
                    <tr>
                   ...
                   <tr>
                     ...
                     <td>:</td>
                     <td>
                      @if($transaction->transactionduedate != null )
                        {{ date('d-m-Y', strtotime($transaction->transactionduedate)) }} 
                      @endif
                     </td>
                   </tr>
                    <tr>
                     ...
                     <td>:</td>
                     <td><b><span id="amount_paid" style="color: green;">{{ $transaction->transactionamount }}</span></b></td>
                   </tr>
                 </table>
