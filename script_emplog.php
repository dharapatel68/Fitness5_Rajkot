<?php
  ...
  if($employeecount > 0){
    ...
    while($row_employee = mysqli_fetch_array($employee, MYSQLI_ASSOC)) {
      ...
      $emplog = mysqli_query($conn, 'SELECT * FROM hr_devicefetchlogs WHERE detail1 = "'.$empid.'" ORDER BY date ASC');
      ...
      while($row_emplog = mysqli_fetch_array($emplog, MYSQLI_ASSOC)) {
        $count++;
        $lastlog = mysqli_query($conn, 'SELECT * FROM hr_employeelog WHERE userid = "'.$empid.'" ORDER BY emplogid DESC LIMIT 1');
        if(!$lastlog){
          echo mysqli_error($conn);
        }
        ...
        if($lastlogcount > 0){
          ...
          if(date('Y-m-d', strtotime($row_emplog['date'])) >= $punchdate){
            if(empty($checkout) && $punchdate == date('Y-m-d', strtotime($row_emplog['date'])) && date('H:i:s', strtotime($row_emplog['time']))> $checkin){
              $updatelog = mysqli_query($conn, 'UPDATE hr_employeelog SET checkout="'.date('H:i:s', strtotime($row_emplog['time'])).'" WHERE emplogid="'.$emplogid.'"');
              ...
              if($row_emplog['date'].' '.$row_emplog['time'] > $checkindate && $row_emplog['date'].' '.$row_emplog['time'] > $checkoutdate){
                ...
                $insertlog = mysqli_query($conn, 'INSERT INTO hr_employeelog (userid, punchdate, checkin, checkout) VALUES ("'.$empid.'", "'.date('Y-m-d', strtotime($row_emplog['date'])).'", "'.date('H:i:s', strtotime($row_emplog['time'])).'", null)');
                  if(!$insertlog){
                    echo mysqli_error($conn);
                  }
              }
